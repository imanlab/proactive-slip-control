...
train_out_dir  = dataset_path + 'train_image_dataset_10c_00h/'
test_out_dir   = dataset_path + 'test_image_dataset_10c_00h/'
scaler_out_dir = dataset_path + 'scalars_00h/'
...
class data_formatter:
    ...
    def create_map(self):
        for stage in [train_out_dir, test_out_dir]:
            ...
            for experiment_number, file in tqdm(enumerate(files_to_run)):
                ...
                tactile, meta, slip, failure = self.load_file_data(file)

                # scale the data
                for index, standard_scaler in enumerate(self.tactile_standard_scaler):
    ...
    def save_map(self, path, test=False):
        if test:
            with open(path + '/map_' + str(self.test_no) + '.csv', 'w') as csvfile:
                ...
                writer.writerow(['tactile_data_sequence', 'experiment_number', 'time_steps', 'meta', 'slip', 'failure'])
    ...
    def scale_data(self):
        ...
        for file in tqdm(files):
            tactile, _, _, _ = self.load_file_data(file)
    ...
    def load_file_data(self, file):
        ...
        return tactile_data, meta_data, slip_label, fail_label
